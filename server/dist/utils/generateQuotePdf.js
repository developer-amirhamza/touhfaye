...
const generateQuotePdf = (data) => {
    return new Promise((resolve, reject) => {
        ...
        const col1X = 50;
        const fmtDate = (d) => new Date(d).toLocaleDateString("en-AU", {
            day: "2-digit",
            month: "long",
            year: "numeric",
        });
        ...
        data.items.forEach((item, i) => {
            ...
            if (sub)
                doc.font("Helvetica").fillColor(mutedGray).fontSize(7.5).text(sub, cols.name + 4, rowY + 16, { width: 160 });
            doc.fillColor(darkGray).fontSize(8.5).font("Helvetica")
        ...
        rowY += 10;
        const addRow = (label, value, bold = false, hl = false) => {
            if (hl)
                doc.rect(totalsX, rowY, totalsWidth, 22).fill(teal);
        ...
        if (data.discount > 0)
            addRow("Annual discount", `-$${data.discount.toFixed(2)}`);
        addRow("Delivery", `$${data.delivery.toFixed(2)}`);
        addRow("GST", `$${data.gst.toFixed(2)}`);
        addRow("TOTAL", `$${data.total.toFixed(2)}`, true, true);
        // ── Footer ──
        doc.fillColor(mutedGray).fontSize(8).font("Helvetica").text("This quote is an estimate of supply based on the usage provided. Continence aids and NDIS supplies may be GST-free. Questions? Contact hello@mytouhfaye.com.au", col1X, rowY + 24, { align: "center", width: pageWidth });
        doc.end();
    });
};
