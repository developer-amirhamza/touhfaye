...
exports.updateEnquiryStatus = exports.listEnquiries = exports.submitMeetingRequest = exports.submitEnquiry = exports.submitFundingEnquiry = void 0;
const errorHandler_1 = require("../utils/errorHandler");
const prisma_1 = require("../lib/prisma");
const sendEmail_1 = require("../config/sendEmail");
const generateQuotePdf_1 = require("../utils/generateQuotePdf");
const TEAM_EMAIL = "hello@mytouhfaye.com.au";
...
const submitFundingEnquiry = async (req, res) => {
    try {
        const userId = req.userId;
        const { participantNeed, availableFunding, message, quoteId, firstName, lastName, email, phone } = req.body;
        ...
        if (quoteId) {
            ...
            if (quote) {
                try {
                    const pdf = await (0, generateQuotePdf_1.generateQuotePdf)({
...
exports.submitFundingEnquiry = submitFundingEnquiry;
...
const submitEnquiry = async (req, res) => {
    try {
        ...
        if (!message)
            return (0, errorHandler_1.errorHandler)(res, 400, "A message is required", true);
        const fullName = firstName + " " + lastName;
        const enquiry = await prisma_1.prisma.enquiry.create({
            data: {
...
const submitMeetingRequest = async (req, res) => {
    try {
        ...
        if (!fullName.trim())
            return (0, errorHandler_1.errorHandler)(res, 400, "First name and surname are required", true);
        if (!email)
            return (0, errorHandler_1.errorHandler)(res, 400, "Email address is required", true);
        if (!phone)
            return (0, errorHandler_1.errorHandler)(res, 400, "A phone number is required so we can call you", true);
        if (!message)
            return (0, errorHandler_1.errorHandler)(res, 400, "Please tell us what you'd like to cover", true);
        const enquiry = await prisma_1.prisma.enquiry.create({
            data: {
                type: "MEETING",
                status: "NEW",
...
exports.submitMeetingRequest = submitMeetingRequest;
...
const updateEnquiryStatus = async (req, res) => {
    try {
        ...
        if (!id || !status)
            return (0, errorHandler_1.errorHandler)(res, 400, "id and status are required", true);
        const enquiry = await prisma_1.prisma.enquiry.update({
            where: { id },
            data: {
                status,
