"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.deleteTeamTask = exports.updateTeamTask = exports.acceptTeamTask = exports.createTeamTask = exports.listTeamTasks = void 0;
const prisma_1 = require("../lib/prisma");
const errorHandler_1 = require("../utils/errorHandler");
const TYPES = ["TASK", "REQUIREMENT", "NOTE"];
const STATUSES = ["OPEN", "IN_PROGRESS", "DONE"];
const PRIORITIES = ["LOW", "NORMAL", "HIGH"];
const requesterName = async (userId) => {
    const user = await prisma_1.prisma.user.findUnique({
        where: { id: userId },
        select: { firstName: true, lastName: true },
    });
    if (!user)
        return "Unknown";
    return `${user.firstName} ${user.lastName ?? ""}`.trim();
};
// List all team tasks/notes, newest first.
const listTeamTasks = async (_req, res) => {
    try {
        const tasks = await prisma_1.prisma.teamTask.findMany({ orderBy: { createdAt: "desc" } });
        return res.status(200).json({ success: true, error: false, data: tasks });
    }
    catch (error) {
        return (0, errorHandler_1.errorHandler)(res, 500, error.message || "Internal server error!", true);
    }
};
exports.listTeamTasks = listTeamTasks;
// Create a task / requirement / note. The writer is the logged-in admin.
const createTeamTask = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            return (0, errorHandler_1.errorHandler)(res, 401, "Authentication required", true);
        const { type, title, description, priority } = req.body;
        if (!title?.trim())
            return (0, errorHandler_1.errorHandler)(res, 400, "Title is required", true);
        if (type && !TYPES.includes(type))
            return (0, errorHandler_1.errorHandler)(res, 400, "Invalid type", true);
        if (priority && !PRIORITIES.includes(priority))
            return (0, errorHandler_1.errorHandler)(res, 400, "Invalid priority", true);
        const task = await prisma_1.prisma.teamTask.create({
            data: {
                type: type ?? "TASK",
                title: title.trim(),
                description: description?.trim() || null,
                priority: priority ?? "NORMAL",
                createdById: userId,
                createdByName: await requesterName(userId),
            },
        });
        return res.status(200).json({ success: true, error: false, message: "Created", data: task });
    }
    catch (error) {
        return (0, errorHandler_1.errorHandler)(res, 500, error.message || "Internal server error!", true);
    }
};
exports.createTeamTask = createTeamTask;
// Accept a task — records the acceptor and moves it to IN_PROGRESS.
// First come, first served: an already-accepted task can't be re-accepted.
const acceptTeamTask = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            return (0, errorHandler_1.errorHandler)(res, 401, "Authentication required", true);
        const { id } = req.body;
        if (!id)
            return (0, errorHandler_1.errorHandler)(res, 400, "id is required", true);
        const name = await requesterName(userId);
        // Conditional update so two admins clicking Accept at once can't both win.
        const updated = await prisma_1.prisma.teamTask.updateMany({
            where: { id, acceptedById: null },
            data: {
                acceptedById: userId,
                acceptedByName: name,
                acceptedAt: new Date(),
                status: "IN_PROGRESS",
            },
        });
        if (updated.count === 0) {
            return (0, errorHandler_1.errorHandler)(res, 409, "This task has already been accepted", true);
        }
        const task = await prisma_1.prisma.teamTask.findUnique({ where: { id } });
        return res.status(200).json({ success: true, error: false, message: "Task accepted", data: task });
    }
    catch (error) {
        return (0, errorHandler_1.errorHandler)(res, 500, error.message || "Internal server error!", true);
    }
};
exports.acceptTeamTask = acceptTeamTask;
// Update content / status / priority.
const updateTeamTask = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            return (0, errorHandler_1.errorHandler)(res, 401, "Authentication required", true);
        const { id, title, description, status, priority, type } = req.body;
        if (!id)
            return (0, errorHandler_1.errorHandler)(res, 400, "id is required", true);
        const existing = await prisma_1.prisma.teamTask.findUnique({ where: { id } });
        if (!existing)
            return (0, errorHandler_1.errorHandler)(res, 404, "Task not found", true);
        if (status && !STATUSES.includes(status))
            return (0, errorHandler_1.errorHandler)(res, 400, "Invalid status", true);
        if (priority && !PRIORITIES.includes(priority))
            return (0, errorHandler_1.errorHandler)(res, 400, "Invalid priority", true);
        if (type && !TYPES.includes(type))
            return (0, errorHandler_1.errorHandler)(res, 400, "Invalid type", true);
        const data = {};
        if (title?.trim())
            data.title = title.trim();
        if (description !== undefined)
            data.description = description?.trim() || null;
        if (priority)
            data.priority = priority;
        if (type)
            data.type = type;
        if (status) {
            data.status = status;
            data.completedAt = status === "DONE" ? new Date() : null;
        }
        const task = await prisma_1.prisma.teamTask.update({ where: { id }, data });
        return res.status(200).json({ success: true, error: false, message: "Updated", data: task });
    }
    catch (error) {
        return (0, errorHandler_1.errorHandler)(res, 500, error.message || "Internal server error!", true);
    }
};
exports.updateTeamTask = updateTeamTask;
// Delete — only the writer can delete their own item (or an OWNER).
const deleteTeamTask = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            return (0, errorHandler_1.errorHandler)(res, 401, "Authentication required", true);
        const { id } = req.body;
        if (!id)
            return (0, errorHandler_1.errorHandler)(res, 400, "id is required", true);
        const existing = await prisma_1.prisma.teamTask.findUnique({ where: { id } });
        if (!existing)
            return (0, errorHandler_1.errorHandler)(res, 404, "Task not found", true);
        const me = await prisma_1.prisma.user.findUnique({ where: { id: userId }, select: { role: true } });
        if (existing.createdById !== userId && me?.role !== "OWNER") {
            return (0, errorHandler_1.errorHandler)(res, 403, "Only the writer (or the owner) can delete this", true);
        }
        await prisma_1.prisma.teamTask.delete({ where: { id } });
        return res.status(200).json({ success: true, error: false, message: "Deleted" });
    }
    catch (error) {
        return (0, errorHandler_1.errorHandler)(res, 500, error.message || "Internal server error!", true);
    }
};
exports.deleteTeamTask = deleteTeamTask;
