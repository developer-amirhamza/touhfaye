...
exports.listWaitlist = exports.joinWaitlist = void 0;
...
const ALERT_TO = process.env.WAITLIST_ALERT_EMAIL ||
...
const joinWaitlist = async (req, res) => {
    try {
        ...
        if (!name?.trim() || !email?.trim() || !postcode?.trim() || !buyerType) {
            return (0, errorHandler_1.errorHandler)(res, 400, "Please fill name, email, postcode and buyer type", true);
        }
        if (!BUYER_TYPES[buyerType]) {
            return (0, errorHandler_1.errorHandler)(res, 400, "Invalid buyer type", true);
        }
        // Basic email shape check.
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) {
            return (0, errorHandler_1.errorHandler)(res, 400, "Please enter a valid email address", true);
        }
        ...
        const entry = await prisma_1.prisma.waitlistEntry.upsert({
...
const listWaitlist = async (_req, res) => {
    try {
        const entries = await prisma_1.prisma.waitlistEntry.findMany({ orderBy: { createdAt: "desc" } });
